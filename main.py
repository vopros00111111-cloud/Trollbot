...
def get_main_keyboard():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="💰 Баланс"), KeyboardButton(text="🎁 Награда")], [KeyboardButton(text="📦 Каталог"), KeyboardButton(text="❓ Помощь")]], resize_keyboard=True)

async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute('CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, balance INTEGER DEFAULT 0, last_claim TEXT, is_admin INTEGER DEFAULT 0)')
        await db.execute('CREATE TABLE IF NOT EXISTS catalog (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, description TEXT, price INTEGER, image_url TEXT)')
        await db.commit()

async def register_user(user_id: int, username: str):
    async with aiosqlite.connect(DB_PATH) as db:
        clean_username = username.replace("@", "") if username else f"user_{user_id}"
        await db.execute('INSERT OR IGNORE INTO users (user_id, username, balance, is_admin) VALUES (?, ?, 0, 0)', (user_id, clean_username))
...
async def check_admin(user_id: int) -> bool:
    ...
    return data[3] == 1 if data and len(data) >= 4 else False
...
async def cmd_help(message: Message):
    await message.answer("📖 **Команды:**\n/start — Регистрация\n/balance — Баланс\n/claim — Награда\n/transfer @юзер сумма — Перевод\n/catalog — Товары\n\n👑 **Админ:**\n/givemoney @юзер сумма\n/takemoney @юзер сумма\n/additem Имя|Описание|Цена|Фото\n/removeitem ID\n/addadmin @юзер\n/removeadmin @юзер", parse_mode="Markdown")

@dp.message(Command("transfer"))
async def cmd_transfer(message: Message):
    parts = message.text.split()
    if len(parts) != 3: return await message.answer("📝 /transfer @username сумма")
    try:
        ...
        if not sender_data: return await message.answer("❌ Сначала /start")
        if sender_data[1] < amount: return await message.answer(f"❌ Мало денег. У тебя {sender_data[1]}")
        
        async with aiosqlite.connect(DB_PATH) as db:
            target = await (await db.execute('SELECT user_id FROM users WHERE username = ?', (target_name,))).fetchone()
        
        if not target: return await message.answer(f"❌ Юзер @{target_name} не найден")
        if target[0] == sender_id: return await message.answer("❌ Себе нельзя")
...
async def cmd_catalog(message: Message):
    ...
    if not items: return await message.answer(" Каталог пуст")
    text = "📦 **КАТАЛОГ:**\n\n"
    for i in items:
        text += f"🔹 **{i[1]}**\n   {i[2]}\n   💰 Цена: {i[3]} монет\n\n"
...
async def cmd_additem(message: Message):
    if not await check_admin(message.from_user.id): return await message.answer(" Только админ")
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2: return await message.answer(" /additem Имя|Описание|Цена|Ссылка")
    try:
        d = parts[1].split("|")
        if len(d) < 3: raise ValueError
        await add_to_catalog(d[0].strip(), d[1].strip(), int(d[2].strip()), d[3].strip() if len(d)>3 else None)
        await message.answer(f"✅ Товар **{d[0]}** добавлен!", parse_mode="Markdown")
    except:
...
async def cmd_removeitem(message: Message):
    if not await check_admin(message.from_user.id): return await message.answer("🔒 Только админ")
    parts = message.text.split()
    if len(parts) != 2: return await message.answer("📝 /removeitem ID")
    try:
...
async def cmd_myitems(message: Message):
    ...
    if not items: return await message.answer(" Каталог пуст")
    text = " **Товары (ID):**\n"
    for i in items: text += f"`{i[0]}` — {i[1]} ({i[3]} монет)\n"
    await message.answer(text, parse_mode="Markdown")
...
async def cmd_givemoney(message: Message):
    if not await check_admin(message.from_user.id): return await message.answer("🔒 Только админ")
    parts = message.text.split()
    if len(parts) != 3: return await message.answer(" /givemoney @username сумма")
    try:
        target_name = parts[1].replace("@", "")
        amount = int(parts[2])        if amount <= 0: return await message.answer("⛔ Сумма > 0")
        ...
        try: await bot.send_message(target[0], f"🎁 Админ выдал **{amount}** монет!\nБаланс: **{new_bal}**", parse_mode="Markdown")
        except: pass
    except ValueError:
        await message.answer("❌ Ошибка формата")
...
async def cmd_takemoney(message: Message):
    if not await check_admin(message.from_user.id): return await message.answer(" Только админ")
    parts = message.text.split()
    if len(parts) != 3: return await message.answer("📝 /takemoney @username сумма")
    try:
        target_name = parts[1].replace("@", "")
        amount = int(parts[2])
        if amount <= 0: return await message.answer("⛔ Сумма > 0")
        ...
        try: await bot.send_message(target[0], f"⚠️ Админ списал **{amount}** монет!\nБаланс: **{new_bal}**", parse_mode="Markdown")
        except: pass
    except ValueError:
        await message.answer("❌ Ошибка формата")
...
async def cmd_addadmin(message: Message):
    if not await check_admin(message.from_user.id): return await message.answer(" Только админ")
    parts = message.text.split()
    if len(parts) != 2: return await message.answer("📝 /addadmin @username")
    try:
        name = parts[1].replace("@", "")
        async with aiosqlite.connect(DB_PATH) as db:
            target = await (await db.execute('SELECT user_id, username FROM users WHERE username = ?', (name,))).fetchone()
        if not target: return await message.answer(f"❌ @{name} не найден")        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute('UPDATE users SET is_admin = 1 WHERE user_id = ?', (target[0],))
            await db.commit()
...
async def cmd_removeadmin(message: Message):
    if not await check_admin(message.from_user.id): return await message.answer("🔒 Только админ")
    parts = message.text.split()
    if len(parts) != 2: return await message.answer("📝 /removeadmin @username")
    try:
        name = parts[1].replace("@", "")
        async with aiosqlite.connect(DB_PATH) as db:
            target = await (await db.execute('SELECT user_id, username FROM users WHERE username = ?', (name,))).fetchone()
        if not target: return await message.answer(f"❌ @{name} не найден")
        async with aiosqlite.connect(DB_PATH) as db:
...
async def btn_balance(m: Message): await cmd_balance(m)
@dp.message(F.text == "🎁 Награда")
async def btn_claim(m: Message): await cmd_claim(m)
@dp.message(F.text == "📦 Каталог")
async def btn_catalog(m: Message): await cmd_catalog(m)
@dp.message(F.text == "❓ Помощь")
async def btn_help(m: Message): await cmd_help(m)
