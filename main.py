...
async def handle_join_poker_table(request):
    try:
        ...
        username = data.get('username', f"Игрок {user_id}")

        if not table_id or not user_id:
            return web.json_response({'success': False, 'error': 'Неполные данные'}, status=400)

        if table_id not in poker_tables:
            return web.json_response({'success': False, 'error': 'Стол не найден'}, status=404)

        table = poker_tables[table_id]

        # Проверяем, не зашел ли игрок, который уже сидит за столом
        player_exists = any(p['user_id'] == user_id for p in table['players'])
        
        if not player_exists:
            if len(table['players']) >= 2:  # Лимит стола (например, хедз-ап на 2 игрока)
                return web.json_response({'success': False, 'error': 'Стол уже заполнен'}, status=400)
        ...
        # --- ЛОГИКА АВТОМАТИЧЕСКОГО СТАРТА ИГРЫ ---
        # Если набралось 2 игрока и игра еще не запущена
        if len(table['players']) == 2 and not table.get('game_started'):
            ...
            # 3. Назначаем ход первому игроку
            table['players'][0]['is_turn'] = True
            
            # 4. Инициализируем состояние стола
            table['deck'] = deck
            table['community_cards'] = []  # На префлопе карт на столе нет
            table['pot'] = 0
            table['current_bet'] = 0
            table['game_started'] = True  # Переключаем флаг старта!
            table['status'] = 'preflop'

        return web.json_response({
            'success': True, 
            'message': 'Вы успешно подключились к столу',
            'game_started': table.get('game_started', False)
        })

    except Exception as e:
        logger.error(f"Ошибка в handle_join_poker_table: {e}")
        return web.json_response({'success': False, 'error': str(e)}, status=500)
    
async def handle_get_poker_table(request):
    table_id = request.match_info.get('table_id')
    if table_id not in poker_tables:
        return web.json_response({'success': False, 'error': 'Стол не найден'}, status=404)
        ...
        'players': [
            {
                ...
                'cards': p.get('cards', [])  # Передаем карты
