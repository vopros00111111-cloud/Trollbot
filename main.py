...
async def handle_join_poker_table(request):
    ...
    if current_count >= max_count:
        ...
        # Создаём запись в active_poker_games — там живут карты и ходы
        game_entry = {
            'uuid': table_id,
            'host': table['host'],
            'bet': table['bet'],
...
async def handle_get_poker_table(request):
    ...
    # Если игра активна — берём данные из active_poker_games (там карты, банк, этап)
    ...
    return web.json_response({
        ...
        'pot': game.get('pot', 0),
        'current_bet': table.get('current_bet', game.get('current_bet', 0)),
        'community_cards': visible_community,
        'my_cards': my_cards,
        'game_started': table['status'] == 'started'
    })
async def handle_poker_action(request):
    """POST /api/poker/action — колл, фолд, рейз из мини-приложения"""
    data = await request.json()
    user_id  = int(data.get('user_id', 0))
    table_id = data.get('table_id', '')
    action   = data.get('action', '')   # 'call' | 'fold' | 'raise'
    ...
    if game.get('finished'):
        return web.json_response({'error': 'Игра завершена'}, status=400)
    if user_id not in game.get('active_players', []):
    ...
    async with poker_locks[table_id]:
        if action == 'fold':
            ...
            if not ok:
                ...
                game.setdefault('responses', set()).add(user_id)
                # Синхронизируем банк в poker_tables
            ...
            # Сбрасываем ответы всех кроме рейзера — они должны ответить снова
    ...
    return web.json_response({'success': True, 'pot': game.get('pot', 0), 'stage': game.get('stage', stage)})
