...
# 🔹 ОТПРАВЛЯЕМ ПРИГЛАШЕНИЕ В ЧАТ
    ...
    # Сохраняем msg_id чтобы обновлять счётчик игроков
    if sent_msg_id:
...
async def handle_join_poker_table(request):
    ...
    if current_count >= max_count:
        ...
        # Раздаём карты
        asyncio.create_task(_deal_poker_cards(table))
...
async def handle_get_poker_table(request):
    ...
    # Обогащаем список игроков username-ами
    enriched_players = []
    ...
    hands = table.get('hands', {})
    my_cards = hands.get(requesting_uid, [])

    # Карты на столе — по этапу
    stage = table.get('stage', 'preflop')
    community = table.get('community', [])
    if stage == 'preflop':
        visible_community = []
    elif stage == 'flop':
