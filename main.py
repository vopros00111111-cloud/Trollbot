...
async def cmd_transfer(message: Message):
    ...
    if len(parts) != 3:
        ...
        return
    try:        target_username = parts[1].replace("@", "")
        amount = int(parts[2])
        if amount <= 0:
...
# ========== АДМИН: ДЕНЬГИ ==========
@dp.message(Command("givemoney"))async def cmd_givemoney(message: Message):
    if not await check_admin(message.from_user.id):
        await message.answer("🔒 Только администратор")
        return
    parts = message.text.split()
...
async def cmd_takemoney(message: Message):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            cursor = await db.execute('SELECT user_id, username, balance FROM users WHERE username = ?', (target_username,))
            target_data = await cursor.fetchone()
        if not target_data:            await message.answer(f"❌ Пользователь @{target_username} не найден")
            return
        target_id, target_username, old_balance = target_data
...
async def cmd_removeadmin(message: Message):
    if not await check_admin(message.from_user.id):
        await message.answer("🔒 Только администратор")
        return
    parts = message.text.split()    if len(parts) != 2:
