...
async def _poker_finish(game: dict):
    ...
    # Сохраняем карты всех игроков для шоудауна (до очистки)
    showdown_snapshot = {}
    for uid, cards in game.get('hands', {}).items():
        uname = next((p['username'] for p in game.get('players', []) if p['user_id'] == uid), str(uid))
        showdown_snapshot[str(uid)] = {'username': uname, 'cards': cards}
    game['showdown_snapshot'] = showdown_snapshot
